...
MODELS = [
    ...
    "repvit_m15", "mambavision_base", "yolo11_cls", "depth_anything_v2", "siglip2_base",
    "tinyvim_base", "sam2_hiera_base", "mobileclip2_b"
]
...
def find_best_ckpt(dataset_output_name, model_type):
    ...
    for date_dir in all_dates:
        target_path = os.path.join(outputs_root, date_dir, exp_name, "checkpoints", "best.pth")
...
def load_merged_config(model_name, dataset_name, source_sub):
    ...
    dataset_cfg_path = os.path.join("configs", "datasets", f"{dataset_name}.yaml")
    ...
    model_cfg_path = os.path.join("configs", "models", dataset_name, source_sub, f"{model_name}.yaml")
...
def save_txt_report(df, txt_file_path):
    ...
    with open(txt_file_path, "w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write(f" SDNET2018 跨域/同域评估全景报告 (Generated: {time.strftime('%Y-%m-%d %H:%M:%S')})\n")
        f.write("=" * 80 + "\n\n")
        ...
        for model_name, group in df.groupby("模型名称"):
            ...
            f.write("-" * 80 + "\n")
            f.write(f"{'权重来源(Source)':<15} {'测试目标(Target)':<15} {'测试类型':<12} {'Accuracy (%)':<15} {'F1-Score':<10}\n")
            f.write("-" * 80 + "\n")
            for _, row in group.iterrows():
                test_type = "同域测试" if row['权重来源(Source)'] == row['测试目标(Target)'] else "跨域测试"
                f.write(f"{row['权重来源(Source)']:<15} {row['测试目标(Target)']:<15} {test_type:<12} {row['Accuracy (%)']:<15.2f} {row['F1-Score']:<10.4f}\n")
            f.write("\n" + "." * 80 + "\n\n")
        ...
        pivot_df = df.pivot(index="模型名称", columns=["权重来源(Source)", "测试目标(Target)"], values="Accuracy (%)")
...
def main():
    ...
    print("="*60)
    print(f" 开始 SDNET2018 全模型 [同域 + 跨域] 矩阵评估")
    print(f" 运行设备: {device}")
    print("="*60)
    ...
    for model_name in MODELS:
        ...
        for source_sub in SUBTYPES:
            ...
            if not best_pth:
                print(f"  [跳过] 未找到 {model_name} 在【{source_dataset_output_name}】下的 best.pth")
                continue
            
            # 预加载与初始化模型
            try:
                _, processor = build_model_and_processor(cfg['model']['name'], num_classes=2, class_names=[])
                model, _ = build_model_and_processor(cfg['model']['name'], num_classes=2, class_names=["0", "1"])
                checkpoint = torch.load(best_pth, map_location=device)
                state_dict = checkpoint["model"] if isinstance(checkpoint, dict) and "model" in checkpoint else checkpoint
                model.load_state_dict(state_dict, strict=False)
                model.to(device)
            except Exception as e:
                print(f"  [错误] 加载模型/权重 {model_name} ({source_sub}) 失败: {e}")
                continue

            # 交叉测试 D, P, W 三个测试集
            for target_sub in SUBTYPES:
                DatasetClass = get_dataset_class(cfg['dataset']['type'])
                target_dataset = DatasetClass(cfg['dataset']['dir'], processor, sub_type=target_sub)
                ...
                _, _, test_dataset = random_split(
                    target_dataset, [train_size, val_size, test_size],
                ...
                acc, precision, recall, f1 = evaluate_on_target(model, test_loader, device)
                
                tag = "同域" if source_sub == target_sub else "跨域"
                print(f"  ↳ [{source_sub}权重 -> {target_sub}测试集] ({tag}) | Acc: {acc*100:.2f}% | F1: {f1:.4f}")
    ...
    if summary_results:
        ...
        txt_save_path = os.path.join(target_dir, "cross_domain_results.txt")
        csv_save_path = os.path.join(target_dir, "cross_domain_results.csv")
